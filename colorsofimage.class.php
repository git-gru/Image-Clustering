...
class ColorsOfImage {
	...
	var $color_map = array();
	var $_palette = array();
	var $_min_percentage = 1.7;
	var $_excluded_colors = array();

	static $hit_pixels = 0;
	static $missed_pixels = 0;

	public function __construct( $image, $precision = 10, $maxnumcolors = 7, $trueper = true ) {
		...
		// echo($this->_excluded_colors[0]);
		// echo($this->getBackgroundColor());
	}
	...
	public function readPixels() {
		...
		try {
			...
			$simg = $outputimg($image);

			$dimg = imagecreatetruecolor($this->width, $this->height); // png ?: gif
			...
			imagecopy($dimg,$simg,0,0,0,0,$this->width,$this->height);
			...
			//         $background = imagecolorallocate($dimg , 240, 240, 240);
			...
			// imagecopy($dimg,$simg,0,0,0,0,$this->width,$this->height);
			if ($stype == 'png' || $stype == 'gif') {
				if ($trnprt_indx >= 0 ) {
					$this->workingImage = ($trnprt_color['alpha'] > 120) ? $simg : $dimg ;
				}
				else {
				$this->workingImage = $dimg ;
				}
				imagepng($this->workingImage, "a.png");
			}			
			else {
				$this->workingImage = $dimg ;
			}
		...
		for( $x = 0; $x < $width; $x += $this->precision ) {
			for ( $y = 0; $y < $height; $y += $this->precision ) {
				...
				$index_color = imagecolorsforindex( $this->workingImage, $index );
				if ($index_color['alpha'] < 120) {
	...
	public function getBackgroundColor( $use_palette = true ) {
		...
		if ( $top_left_color['alpha'] > 120 && $top_right_color['alpha'] > 120 && $bottom_left_color['alpha'] > 120 && $bottom_right_color['alpha'] > 120) {
			return null;
		}
	...
	private function getPalette() {
		...
		$str = '["#FFFFFF", "#000000", "#cc0000", "e41087", "#ef5734", "#ff9900", "#efe20f", "#005131", "#02edb9", "#04be94", "#0cb14b", "#a4c639", "#0ea0c0", "#0089cf", "#14407f", "#6460aa", "#6a2d66", "#cecece", "#6d6d6d", "#543729"]';
